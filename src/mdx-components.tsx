...
const MdxComponents: MDXComponents = {
  ...
  Image: (props: any) => {
    ...
    return <ZoomImage {...props} blurDataURL={props.blurDataURL} />;
  },
  ...
  HelpArticles: (props: { articles: string[] }) => (
    <div className="not-prose grid gap-2 rounded-xl border border-gray-200 bg-white p-4">
      {(props.articles || HELP_ARTICLES).map((slug) => (
        <HelpArticleLink
          key={slug}
          article={{
            slug: getPopularArticles(slug)?.slug!,
            title: getPopularArticles(slug)?.title!,
          }}
        />
      ))}
    </div>
